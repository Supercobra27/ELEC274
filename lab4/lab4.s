...
stw r3, 8(sp)
stw r4, 4(sp) # save value of register r3 so it can be a temp
stw r5, 0(sp) # save value of register r4 so it can be a temp
movia r3, JTAG_UART_BASE # point to first memory-mapped I/O register
getchar_loop:
ldwio r4, DATA_OFFSET(r3) # read bits from status register
andi r5, r4, 0x8000 # mask off lower bits to isolate status bit (15)
beq r5, r0, getchar_loop # if status bit is zero, loop again
andi r2, r4, 0x00FF # Extract character from lower 8 bits

ldw r3, 8(sp)
ldw r4, 4(sp) # restore value of r4 from stack
ldw r5, 0(sp) # restore value of r5 from stack
addi sp, sp, 12
